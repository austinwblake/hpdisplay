...
public class SerialConnectionManager
{
    ...
    public void setupSerialPort()
    {
        if (serialPort == null || !serialPort.isOpen())
        {
            ...
            {
                client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "Connected to Adruino through Serial Port: " + config.serialPort() + ".", null, true);
                sendHpInfo(client.getRealSkillLevel(Skill.HITPOINTS), client.getBoostedSkillLevel(Skill.HITPOINTS));
            }
            else
            {
                client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "Unable to connect to Adruino through Serial Port: " + config.serialPort() + ".", null, true);
            }
        }
    }
